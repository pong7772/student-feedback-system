...
const page = ({ params }) => {
    ...
    const fetchProfile = async () => {
        await fetchData(
            "/user/get-user?id=" + params?.userId, {}, "GET"
        ).then((res) => {
            setProfile({
                ...res,
...
export async function generateStaticParams() {
    const res = await fetchData(
        "/user/get-all-user?page=0&size=100&role=ALL", {},
        "GET"
    )
    return res?.content?.map((content) => {
        return {
            userId: content?.accountId
        }
    })
}
